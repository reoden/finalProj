...
func getLocalBucket() (*oss.Bucket, error) {
	...
	ossAccessSecretKey := os.Getenv("OSSaccessSecretKey")
	client, err := oss.New("https://file.gcfbguide.com", ossAccessKeyID, ossAccessSecretKey, oss.UseCname(true))
	if err != nil {
		// HandleError(err)
		return nil, err
	}

	bucket, err := client.Bucket("gcnrg-file")
	if err != nil {
		// HandleError(err)
		return nil, err
	}
