...
func (cr *GuestRepo) GetAppsName(ctx context.Context, names []string) ([]*entity.Guest, error) {
	var app []*entity.Guest
	nameTmp := make([]string, 0)
	for _, str := range names {
		name := fmt.Sprintf("'%s'", str)
		nameTmp = append(nameTmp, name)
	}
	nameStr := strings.Join(nameTmp, ",")
	err := cr.db.Table(TableGuest).
		Where("name IN ?", names).
		Where("status != ?", entity.StatusDeleted).
		Order(fmt.Sprintf("FIELD(name, %s)", nameStr)).
		Find(&app).
		Error
