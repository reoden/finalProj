    ...
    <div v-else class="content">
      ...
      >
        <div class="title">
          ...
          <div class="subtitle">{{ $t('menu.news.title') }}</div>
...
<script lang="ts" setup>
  ...
  const rules: any = {
    name: [
      {
        required: true,
        message: t('news.form.name.validator'),
      },
    ],
    describe: [
      {
        required: true,
        message: t('news.form.describe.validator'),
      },
    ],
    pictures: [
      {
        required: true,
        message: t('news.form.picture.validator'),
      },
    ]
  };
  ...
  const handleSubmit = async() => {
    await formRef.value.validate(async (valid: any) => {
        ...
        toggleBtnLoading();
        try {
          const { data: { code } } = await SavorApi.addNews(form.value);
          if (code === 0) {
            handleCancel();
