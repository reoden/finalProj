  ...
  <div class="container">
    ...
    <div class="content">
      <div class="header">
        ...
        <a-button @click="addNews">增加资讯</a-button>
      </div>
      <div class="nameEdit">
        <div>景点页签修改</div>
        <a-input class="input" v-model="newsName" :disabled="!isEdit"></a-input>
...
import { useRouter } from 'vue-router';
      ...
      if (code === 0) {
        ...
        const drag = new Sortable(tbody, {
          ...
          onEnd: async (evt) => {
            ...
            // 拖拽后接口保存排序后的位置
            const tableData = JSON.parse(JSON.stringify(newsList.value));
            const targetRow = tableData[oldIndex];
            tableData.splice(oldIndex, 1)
            tableData.splice (newIndex, 0, targetRow);
            newsList.value = tableData;
            try {
              const appIds = tableData.map((item: any) => item.id);
              const { data: { code: codeSec  } } = await SavorApi.changeNews({ app_ids: appIds });
              if (codeSec !== 0) {
                // 保存失败后，撤回本次拖拽
                returnDrop(evt);
              }
  ...
  const del = (row?: any) => {
    Modal.confirm({
      content: `确认删除资讯 ${row.name} 吗?`,
      okText: '确定',
      async onOk() {
        const { data: { code } } = await SavorApi.deleteNews(row.id);
        if (code === 0) {
  ...
  const editName = async() => {
    if (isEdit.value) {
      toggleEditLoading();
      try {
        const { data: { code } } = await SavorApi.renameMenu({ news_name: newsName.value });
        if (code === 0) {
          isEdit.value = false;
        }
      } finally {
