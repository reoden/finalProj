    ...
    <div v-else class="content">
      ...
      >
        <div class="title">
          ...
          <div class="subtitle">{{ $t('menu.guest.title') }}</div>
        ...
        <a-form-item field="name" :label="$t('guest.colunm.name')" validate-trigger="blur">
          <a-input v-model="form.name" :placeholder="$t('guest.form.name.placeholder')" />
        </a-form-item>
        <a-form-item field="describe" :label="$t('guest.colunm.describe')" validate-trigger="blur">
          <a-textarea v-model="form.describe" :placeholder="$t('guest.form.describe.placeholder')" :auto-size="false" />
        </a-form-item>
      </a-form>
    </div>
  </div>
...
<script lang="ts" setup>
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { data: { code, body } } = await SavorApi.guestDetail(id);
      if (code === 0) {
        form.value = body;
  ...
  const rules: any = {
    name: [
      {
        required: true,
        message: t('guest.form.name.validator'),
      },
    ],
    describe: [
      {
        required: true,
        message: t('guest.form.describe.validator'),
      },
    ],
    pictures: [
      {
        required: true,
        message: t('guest.form.picture.validator'),
      },
    ]
  };
  ...
  const handleSubmit = async() => {
    await formRef.value.validate(async (valid: any) => {
        ...
        toggleBtnLoading();
        try {
          const { data: { code } } = await SavorApi.addGuest(form.value);
          if (code === 0) {
            handleCancel();
