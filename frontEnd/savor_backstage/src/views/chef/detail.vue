    ...
    <div v-else class="content">
      ...
      >
        <div class="title">
          ...
          <div class="subtitle">{{ $t('menu.chef.title') }}</div>
        ...
        <a-form-item field="name" :label="$t('chef.colunm.name')" validate-trigger="blur">
          <a-input v-model="form.name" :placeholder="$t('chef.form.name.placeholder')" />
        </a-form-item>
        <a-form-item field="address" :label="$t('chef.colunm.address')" validate-trigger="blur">
          <a-input v-model="form.address" :placeholder="$t('chef.form.address.placeholder')" />
        </a-form-item>
        <a-form-item field="describe" :label="$t('chef.colunm.describe')" validate-trigger="blur">
          <a-textarea v-model="form.describe" :placeholder="$t('chef.form.describe.placeholder')" :auto-size="false" />
        </a-form-item>
      </a-form>
    </div>
  </div>
...
<script lang="ts" setup>
  ...
  const rules: any = {
    ...
    address: [
      {
        required: true,
        message: t('chef.form.address.validator'),
      },
    ],
    describe: [
      {
  ...
  const handleSubmit = async() => {
    await formRef.value.validate(async (valid: any) => {
        ...
        toggleBtnLoading();
        try {
          const { data: { code } } = await SavorApi.addChef(form.value);
          if (code === 0) {
            handleCancel();
