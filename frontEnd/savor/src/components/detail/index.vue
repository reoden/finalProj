      ...
      <div class="detailInfo">
        ...
        <div class="shop">
          ...
          <div class="diet">
            ...
            {{ data.have_vege ? $t('settled.form.have_vege.yes') : $t('settled.form.have_vege.no') }}
          </div>
        </div>
      </div>
      <a-divider />
      ...
      <div class="info">
        ...
        <div class="info-detail">
          ...
          <div class="item">
            ...
            <div class="desc">
              {{ Number(data.yuyue) === 1 ? $t('settled.form.yuyue.option1') : $t('settled.form.yuyue.option2') }}
            </div>
          </div>
          <div class="item">
          ...
          <div class="item">
            ...
            <div class="desc">{{ data.work_begin_at }} ~ {{ data.work_end_at}}</div>
          </div>
          <div class="item">
            <div class="label">{{ $t('settled.form.open_lable') }}</div>
            <div class="desc"> {{ data.work_date_label  && data.work_date_label.join('、') }}</div>
          </div>
          <div class="item">
            <div class="label">{{ $t('settled.form.per') }}</div>
            <div class="desc"> {{ data.per }}{{ $t('settled.form.per.desc')}}</div>
          </div>
          <div class="item">
            <div class="label">{{ $t('settled.form.have_vege') }}</div>
            <div class="desc">{{ data.have_vege ? $t('settled.form.have_vege.option1') : $t('settled.form.have_vege.option2') }}</div>
          </div>
        </div>
      </div>
    </div>
  </div>
  <Mobile v-else :detailData="data" />
</template>
...
<script lang="ts" setup>
  ...
  const emit = defineEmits(['showDetail', 'cancel']);
